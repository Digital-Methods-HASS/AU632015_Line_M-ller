...
author: "Line Stampe-Degn Moeller"
date: "Created 8 October, 2021, updated `r format(Sys.time(), '%d %B, %Y')`"
output: 
  html_document:
    toc: true
    toc_float: true
...
colnames(Western_Europe)[4]<-"Homicide"
```
...
## I ONLY MADE ALTERATIONS TO THE FIRST GRAPH AND NOT THE SECOND AS IT DIDN'T SEEM NECESSARY.
...
##In order to relabel the coloumn name I choose colomn [4] and rename it to 'Homicides'
...
##I use the geom_line command to create a better type of visualisation for a long-term trend. 

3) Can you change the facet layout to two columns and three rows so that the trends are more visible? 

##Yes, I use the facetwrap command to do this.

4) Move the legend from the current position on the side to below the facets, and label it "Country" instead of "Entity".

##In order to rename the color in the facet, I use the labs command to rename 'Entity' to 'Country'. I position the legend at the bottom using the theme command.
